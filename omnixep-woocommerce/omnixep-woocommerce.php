...
function wc_omnixep_deactivate()
{
    ...
    error_log('âœ… Terms acceptance data cleared successfully');
    error_log('âš ï¸Â User must re-accept terms on reactivation');
...
function wc_omnixep_terms_notice()
{
    ...
    if (!wc_omnixep_check_terms_acceptance()) {
        ...
        <div class="notice notice-error is-dismissible" style="border-left-width: 5px; border-left-color: #d63638; padding: 20px;">
            <h2 style="margin-top: 0;">âš   OmniXEP Payment Gateway - Terms of Service Required</h2>
            <p style="font-size: 14px; line-height: 1.6;">
            ...
            <ul style="font-size: 13px; color: #666; line-height: 1.8;">
                <li>âœ… 0.8% commission fee structure</li>
                <li>âœ… Security responsibilities and wallet management</li>
                <li>âœ… Liability limitations and risk acknowledgments</li>
                <li>âœ… Legal protections for both merchant and developer</li>
            </ul>
            <p style="margin-top: 15px;">
                <a href="<?php echo admin_url("admin.php?page=omnixep-terms"); ?>" class="button button-primary" style="background: #d63638; border-color: #d63638; font-size: 14px; height: auto; padding: 10px 20px;">
                    ğŸ“„ Read & Accept Terms of Service
                </a>
            </p>
        </div>
        <?php
        ...
        if (!$synced && isset($_GET['section']) && $_GET['section'] === 'omnixep') {
            ...
            <div class="notice notice-info is-dismissible" style="border-left-width: 5px; border-left-color: #4dabf7; padding: 15px;">
                <p style="margin: 0;">
                    <strong>â„¹ï¸  Terms Acceptance Not Synced:</strong> 
...
function wc_omnixep_render_terms_page()
{
    ...
    if (isset($_POST['omnixep_accept_terms']) && check_admin_referer('omnixep_accept_terms')) {
        if (isset($_POST['accept_checkbox']) && $_POST['accept_checkbox'] === '1') {
            ...
            error_log('âœ… Terms acceptance saved to WordPress options (including text snapshot)');
            ...
            error_log('âš ï¸Â OMNIXEP TERMS ACCEPTANCE FAILED: Checkbox not checked');
    ...
    <div class="wrap" style="max-width: 1200px; margin: 20px auto;">
        <h1 style="font-size: 28px; margin-bottom: 20px;">ğŸ“„ OmniXEP Payment Gateway - Terms of Service</h1>
        ...
        <form method="post" action="" style="background: #fff; border: 2px solid #d63638; border-radius: 4px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
            ...
            <div style="margin-bottom: 25px; padding: 20px; background: #fff9e6; border-left: 4px solid #f1c40f; border-radius: 4px;">
                <h3 style="margin-top: 0; color: #856404;">âš ï¸Â Important Acknowledgments</h3>
                ...
                <ul style="font-size: 13px; line-height: 2; color: #666;">
                    <li>âœ… <strong>Software License Only:</strong> This is a software tool, not a payment processor</li>
                    <li>âœ… <strong>No Custody:</strong> Developer never has access to your funds or private keys</li>
                    <li>âœ… <strong>0.8% Commission Fee:</strong> Software service fee on all transactions</li>
                    <li>âœ… <strong>Security Responsibility:</strong> You are solely responsible for securing your mnemonic phrase</li>
                    <li>âœ… <strong>Blockchain Risks:</strong> Transactions are irreversible and subject to network conditions</li>
                    <li>âœ… <strong>Regulatory Compliance:</strong> You are responsible for legal and tax compliance</li>
                    <li>âœ… <strong>Limited Liability:</strong> Maximum liability is 100 USD or 30 days of license fees paid (whichever is lower)</li>
                    <li>âœ… <strong>Governing Law:</strong> Republic of TÃ¼rkiye â€“ Ä°stanbul Courts and Enforcement Offices</li>
                </ul>
            </div>
...
function wc_omnixep_render_sync_page()
{
    ...
    <div class="wrap" style="max-width: 1000px; margin: 20px auto;">
        <h1 style="font-size: 28px; margin-bottom: 20px;">ğŸ”„ OmniXEP Terms Acceptance - API Sync</h1>
        
        <?php if (isset($sync_message)): ?>
            <div class="notice notice-success" style="padding: 15px; margin-bottom: 20px;">
                <p><strong>âœ… Success:</strong> <?php echo esc_html($sync_message); ?></p>
            </div>
        <?php endif; ?>
        
        <?php if (!$terms_accepted): ?>
            <div class="notice notice-error" style="padding: 20px; margin-bottom: 20px;">
                <h2 style="margin-top: 0;">âš ï¸Â Terms Not Accepted</h2>
        ...
        <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($synced_to_api): ?>
                                    <span style="color: #2ecc71; font-weight: 600;">âœ… Yes</span>
                                <?php else: ?>
                                    <span style="color: #e74c3c; font-weight: 600;">âŒ No</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div style="background: #fff; border: 1px solid #ccd0d4; border-radius: 4px; padding: 30px; margin-bottom: 20px;">
                <h2 style="margin-top: 0; color: #2c3e50;">ğŸ”„ Manual Sync to API</h2>
                
                <?php if ($synced_to_api): ?>
                    <div style="background: #e7f5ff; border-left: 4px solid #4dabf7; padding: 15px; margin-bottom: 20px;">
                        <p style="margin: 0;">
                            <strong>â„¹ï¸  Already Synced:</strong> Your terms acceptance has already been sent to the API.
                ...
                <?php else: ?>
                    <div style="background: #fff9e6; border-left: 4px solid #f1c40f; padding: 15px; margin-bottom: 20px;">
                        <p style="margin: 0;">
                            <strong>âš ï¸Â Not Synced:</strong> Your terms acceptance hasn't been sent to the API yet.
                ...
                <form method="post" action="">
                    ...
                    <button type="submit" name="omnixep_manual_sync" class="button button-primary" style="font-size: 16px; padding: 12px 30px; height: auto;">
                        ğŸ”„ <?php echo $synced_to_api ? 'Re-Sync' : 'Sync'; ?> to API Now
                    </button>
                </form>
            </div>
            
            <div style="background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 4px; padding: 20px;">
                <h3 style="margin-top: 0; font-size: 16px;">ğŸ“– How to View in Firebase/API</h3>
...
function wc_omnixep_send_terms_acceptance_to_api($acceptance_date, $user_id, $ip_address)
{
    ...
    if (is_wp_error($response)) {
        error_log('âŒ TERMS ACCEPTANCE API ERROR: ' . $response->get_error_message());
        ...
        error_log('âœ… TERMS ACCEPTANCE API SYNC SENT SUCCESSFULLY');
...
function wc_omnixep_sync_existing_terms_to_api()
{
    ...
    error_log('âœ… EXISTING TERMS ACCEPTANCE SYNCED TO API SUCCESSFULLY');
...
function wc_omnixep_security_monitor_page() {
    ...
    ?>
    <div class="wrap">
        <h1>ğŸ”’ OmniXEP Security Monitor</h1>
        ...
        <div style="background: #fff3cd; border: 1px solid #ffc107; border-radius: 5px; padding: 20px; margin: 20px 0;">
            <h3 style="margin-top: 0; color: #856404;">âš ï¸ Security Recommendations</h3>
        ...
        <div style="background: #e7f3ff; border: 1px solid #0073aa; border-radius: 5px; padding: 15px; margin: 20px 0;">
            <p style="margin: 0; font-size: 12px; color: #0073aa;">
                â„¹ï¸ This page displays the last 100 security events. Events are automatically rotated to keep the database clean.
...
function wc_omnixep_frontend_configuration_warning()
{
    ...
    <div id="omnixep-extension-warning" class="woocommerce-notice woocommerce-notice--error woocommerce-error" role="alert"
        ...
        <div style="display: flex; align-items: flex-start; gap: 12px;">
            <span style="font-size: 24px; line-height: 1;">âš ï¸Â</span>
            <div>
                    ...
                    if (el.children(':not(br, span.omnixep-verified, a.omnixep-txid-link)').length === 0) {
                        ...
                        if (hashPattern.test(html)) {
                            var newHtml = html.replace(/\(\s*([a-fA-F0-9]{64})\s*\)/g, function (match, hash) {
                                ...
                                var link = '<span class="omnixep-verified">âœ“</span><a href="https://electraprotocol.network/transaction/' + hash + '" target="_blank" class="omnixep-txid-link">(' + shortHash + ')</a>';
                                return link;
                            });
                            if (html !== newHtml) {
                                el.html(newHtml);
...
function wc_omnixep_admin_order_preview_details($data, $order)
{
    ...
    if ($order->get_payment_method() === 'omnixep') {
        ...
        if ($txid) {
            ...
            $check_icon = '<span style="color:#28a745;">âœ“</span>';
...
function wc_omnixep_payment_meta_box_content($post_or_order)
{
    ...
    if ($token_name && $amount) {
        ...
        if ($commission_amount && floatval($commission_amount) > 0) {
            ...
            echo '<p style="margin: 3px 0; font-size: 0.95em;"><strong>âš ï¸Â Two-Step Payment:</strong></p>';
    ...
    if ($commission_txid) {
        ...
        echo '<span style="color: #28a745; font-size: 16px; margin-right: 5px;">âœ“</span>';
        echo '<a href="' . esc_url($comm_link) . '" target="_blank" style="color: #155724;">';
        echo esc_html(substr($commission_txid, 0, 20)) . '...';
        echo '</a>';
        echo '</div>';
        echo '<p style="margin-top: 8px;"><a href="' . esc_url($comm_link) . '" target="_blank" class="button button-small">ğŸ” View Commission TX</a></p>';
    } elseif ($commission_amount && floatval($commission_amount) > 0) {
        echo '<p style="color: #dc3545; margin: 10px 0;"><strong>âš ï¸Â STEP 1 - Commission TX:</strong> Not received yet (required first!)</p>';
    }
    ...
    if ($txid) {
        ...
        echo '</a>';
        echo '</div>';
        echo '<p style="margin-top: 8px;"><a href="' . esc_url($tx_link) . '" target="_blank" class="button button-small">ğŸ” View Order TX</a></p>';
    } else {
        echo '<p style="color: #dc3545; margin: 10px 0;"><strong>âš ï¸Â STEP 2 - Order TX:</strong> Not received</p>';
    }
    ...
    if ($total_debt > 0) {
        ...
        echo '<p style="margin: 0; font-size: 0.9em; color: #555;"><strong>ğŸ“¡ 0.8% Sales Commission:</strong></p>';
...
function wc_omnixep_format_payment_method_title($title, $order)
{
    ...
    if ($txid) {
        ...
        $check_icon = $verified ? '<span style="color:#28a745;">âœ“</span>' : '<span style="color:#ffc107;">â³</span>';
        $result .= ' ' . $check_icon;
    }
...
function wc_omnixep_submit_feedback($data)
{
    ...
    if (!empty($result['success'])) {
        error_log('âœ… Customer Feedback Submitted to API: ' . ($result['reference_number'] ?? 'N/A'));
    } else {
        error_log('âŒ Customer Feedback Failed: ' . ($result['error'] ?? 'Unknown error'));
    }
                ...
                <form id="xep-feedback-form" class="xep-feedback-form">
                    ...
                    <div style="background: rgba(0,242,255,0.1); border-left: 3px solid #00f2ff; padding: 12px 16px; border-radius: 8px; margin-bottom: 25px;">
                        <p style="margin: 0; font-size: 13px; color: rgba(255,255,255,0.8); line-height: 1.6;">
                            <strong style="color: #00f2ff;">â„¹ï¸ Important:</strong> 
...
function omnixep_sync_feedback_to_api_handler($feedback_id)
{
    ...
    if (!is_wp_error($response)) {
        error_log('âœ… Feedback synced to API: ' . $feedback['reference_number']);
    }
}
