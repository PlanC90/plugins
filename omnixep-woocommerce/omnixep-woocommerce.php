...
require_once plugin_dir_path(__FILE__) . 'includes/omnixep-i18n.php';
...
/**
 * Global Anti-Translation Security Meta Tags
 * Blocks Google / Edge / Chrome automatic translation engines from altering
 * sensitive DOM elements that are signed via HMAC.
 */
function wc_omnixep_add_notranslate_meta() {
    // Only apply on OmniXEP specific pages or checkout/cart to minimize global impact
    $screen = is_admin() ? get_current_screen() : null;
    $is_omnixep_admin = $screen && (strpos($screen->id, 'omnixep') !== false || (isset($_GET['page']) && $_GET['page'] === 'wc-settings' && isset($_GET['section']) && $_GET['section'] === 'omnixep'));
    
    if (is_checkout() || is_cart() || $is_omnixep_admin || (isset($_GET['page']) && strpos($_GET['page'], 'omnixep') !== false)) {
        echo '<meta name="google" content="notranslate" />' . "\n";
    }
}
add_action('wp_head', 'wc_omnixep_add_notranslate_meta', 1);
add_action('admin_head', 'wc_omnixep_add_notranslate_meta', 1);

/**
 * Add 'notranslate' class to body on sensitive pages
 */
function wc_omnixep_body_class_notranslate($classes) {
    if (is_checkout() || is_cart()) {
        $classes[] = 'notranslate';
    }
    return $classes;
}
add_filter('body_class', 'wc_omnixep_body_class_notranslate');

// For Admin body class
add_filter('admin_body_class', function($classes) {
    $screen = get_current_screen();
    if ($screen && (strpos($screen->id, 'omnixep') !== false || (isset($_GET['section']) && $_GET['section'] === 'omnixep'))) {
        $classes .= ' notranslate';
    }
    return $classes;
});
...
/**
 * Universal Admin Anti-Translation Reinforcement
 */
add_action('admin_footer', function() {
    $screen = get_current_screen();
    $is_omnixep = $screen && (strpos($screen->id, 'omnixep') !== false || (isset($_GET['section']) && $_GET['section'] === 'omnixep'));
    
    if (!$is_omnixep) return;
    ?>
    <script>
        (function() {
            const reinforceNotranslate = function() {
                const selectors = ['input', 'textarea', 'code', 'pre', '.notranslate', '.omnixep-required-field'];
            ...
            reinforceNotranslate();
            setInterval(reinforceNotranslate, 2000);
        })();
    </script>
    <?php
});
...
function wc_omnixep_terms_notice()
{
    ...
    if (!wc_omnixep_check_terms_acceptance()) {
        ?>
        <div class="notice notice-error is-dismissible notranslate" translate="no" style="border-left-width: 5px; border-left-color: #d63638; padding: 20px;">
        ...
        if (!$synced && isset($_GET['section']) && $_GET['section'] === 'omnixep') {
            ?>
            <div class="notice notice-info is-dismissible notranslate" translate="no" style="border-left-width: 5px; border-left-color: #4dabf7; padding: 15px;">
                <p style="margin: 0;">
...
function wc_omnixep_render_terms_page()
{
    ...
    ?>
    <div class="wrap notranslate" translate="no" style="max-width: 1200px; margin: 20px auto;">
        <script>
            // Protection against browser auto-translation corruption
            (function() {
                const termsProtection = function() {
                    // Target all sensitive legal elements and form inputs
                    const elements = document.querySelectorAll('input, select, textarea, code, pre, .notranslate, blockquote, .wrap');
                    elements.forEach(el => {
                        if (el.getAttribute('translate') !== 'no') el.setAttribute('translate', 'no');
                        if (el.getAttribute('notranslate') !== 'true') el.setAttribute('notranslate', 'true');
                        if (!el.classList.contains('notranslate')) el.classList.add('notranslate');
                    });
                };
                document.addEventListener('DOMContentLoaded', termsProtection);
                window.addEventListener('load', termsProtection);
                // Aggressive check every 1.5 seconds to catch delayed translations
                setInterval(termsProtection, 1500);
            })();
        </script>
        ...
        <form method="post" action="" class="notranslate" translate="no" style="background: #fff; border: 2px solid #d63638; border-radius: 4px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
...
function wc_omnixep_render_sync_page()
{
    ...
    <div class="wrap notranslate" translate="no" style="max-width: 1000px; margin: 20px auto;">
        <script>
            ...
            (function() {
                const termsProtection = function() {
                    const elements = document.querySelectorAll('input, select, textarea, code, pre, .notranslate, table, .wrap');
                    elements.forEach(el => {
                        if (el.getAttribute('translate') !== 'no') el.setAttribute('translate', 'no');
                        if (el.getAttribute('notranslate') !== 'true') el.setAttribute('notranslate', 'true');
                        if (!el.classList.contains('notranslate')) el.classList.add('notranslate');
