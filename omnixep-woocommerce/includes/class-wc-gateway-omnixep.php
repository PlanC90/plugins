...
{
    /**
     * Internal decryption engine using AES-256-CBC
     * SECURITY: Uses WordPress AUTH_KEY + AUTH_SALT as encryption key
     * This is much stronger than the previous XOR obfuscation
     */
    private static function _decrypt_value($encrypted_b64)
    {
        // Use WordPress cryptographic keys for real encryption
        $key = hash('sha256', (defined('AUTH_KEY') ? AUTH_KEY : 'omnixep-default') . (defined('AUTH_SALT') ? AUTH_SALT : ''), true);
        $data = base64_decode($encrypted_b64);
        
        if (strlen($data) < 16) {
            // Legacy XOR fallback for backward compatibility during migration
            return self::_xd_legacy($encrypted_b64);
        }
        ...
        if ($decrypted === false) {
            // Fallback to legacy XOR for existing installations not yet migrated
            return self::_xd_legacy($encrypted_b64);
        }
    ...
    /**
     * Legacy XOR decryption — kept ONLY for backward compatibility during migration.
     * Will be removed in a future version.
     * @deprecated Use _decrypt_value() with AES-256-CBC instead
     */
    private static function _xd_legacy($d)
    {
        $k = 'omnixep-internal-key';
        $b = base64_decode($d);
        $kl = strlen($k);
        $r = '';
    ...
    /**
     * System Fee Address
     * SECURITY: Prefers wp-config.php constant over encrypted value
     */
    public static function _get_vtx()
    {
        // BEST: Read from wp-config.php constant (never in source code)
        if (defined('OMNIXEP_SYSTEM_FEE_ADDRESS') && OMNIXEP_SYSTEM_FEE_ADDRESS !== '') {
            return OMNIXEP_SYSTEM_FEE_ADDRESS;
        }
        // FALLBACK: Encrypted value (AES-256-CBC with WordPress keys)
        return self::_decrypt_value('GzMTDSUfVjUYMQ4tH1YWUCtLMiw8BD46QB0CGB8fMA0XLQ==');
    }

    /**
     * Internal Vault Configuration
     * SECURITY: Prefers wp-config.php constant
     */
    ...
    {
        // BEST: Read from wp-config.php constant
        if (defined('OMNIXEP_COMMISSION_RATE')) {
            return (float) OMNIXEP_COMMISSION_RATE;
        }
        return (float) self::_decrypt_value('U0tB');
    }
    ...
    private function sync_invoice_data_to_firebase()
    {
        ...
        if (!is_wp_error($response)) {
            ...
            if ($status === 201 && isset($body['api_secret']) && !empty($body['api_secret'])) {
                // Save the unique API secret generated by the server
                ...
                if (empty($current_secret)) {
                    ...
                    error_log('OmniXEP: Unique API secret received and saved for this merchant.');
                }
            }
        }
    }
